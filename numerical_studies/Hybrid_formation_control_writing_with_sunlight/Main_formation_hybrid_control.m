...
% maintenance_cost_1 = load('fuel_consumption_first_conf_maintenance_1month.mat');
% maintenance_cost_2 = load('fuel_consumption_second_conf_maintenance_1month.mat');
...
demonstration{1,1}.reconfiguration_time = datetime(2021, 09, 24, 12, 0, 0);
...
demonstration{2,1}.deployment_time = datetime(2021, 09, 24, 12, 0, 0);
demonstration{2,1}.reconfiguration_time = MD.final_orbit_epoch;
...
% Q = diag([2.63118656e-07, 2.63118656e-07, 2.63118656e-07,...
%     9.98107276e-04, 9.98107276e-04, 9.98107276e-04]);
% 2.63118656e-07, 9.98107276e-04
...
%% Cost matrices
% mode = 1 - calculating new cost matrix
% mode = 2 - load the calculated one
mode = 2;
if mode == 1    
    rv_init = formation.rv;
    dt = [0 seconds(demonstration{2,1}.deployment_time - formation.orbit_epoch)];
    ...
    rv_orb_at_reconf = get_rv_from_analytic_HCW_solution(rv_ECI_target, demonstration{1, 1}.HCW_constants, consts);
    for i = 1:formation.N_sats
        rv_ECI_at_reconf(i*6-5:i*6,1) = orb2ECI(rv_ECI_target, rv_orb_at_reconf(:,i), consts);
    end
    tic;
    cost_matrix_dV = get_cost_matrix(demonstration{1,1}, demonstration{2,1}, formation, spacecraft, consts);
%     qqq = cost_matrix_calculation_test(rv_ECI_at_reconf, demonstration{2,1}, formation, spacecraft, consts);
    toc;
    save('C:\GoogleDrive\SatelliteFormationFlying\data\Cost_matrix_JASR', 'cost_matrix_dV');
elseif mode == 2
...
demonstration{1,1}.Cost_matrix_dV = zeros(formation.N_active_sats,formation.N_active_sats);
