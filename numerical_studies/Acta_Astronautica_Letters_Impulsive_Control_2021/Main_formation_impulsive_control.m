...
spacecraft.DragArea = 320e-3*200e-3;                        % [m^2] cross-section area perpendicular to the incoming airflow
...
%         [t_vec_drifting, rv_ECI_drifting] = ode45(@(t, rv) J2_atmo_Formation(t, rv, consts, spacecraft), 0:formation.drifting_time, rv_ECI_undocking(:,end), options_precision);
        [t_vec_drifting, rv_ECI_drifting] = ode45(@(t, rv) central_gravity_Formation(t, rv, consts), 0:formation.drifting_time, rv_ECI_undocking(:,end), options_precision);
    ...
    case 2        
     % for now this is the only feature 
        for i = 1:formation.N_sats
