function [t, rv_ECI, maneuvers_out2, formation_fuel] = multisatellite_orbit_correction_3_impulse(rv_ECI, consts, spacecraft, formation, mode)

% 1. Correcting dsma
% 2. Correction difference in equinoctial oe expcept for sma
    if mode ~= 3
%         disp('    Impulsive control');
    end
    T_local = 0;
    t = 0;
    maneuvers = [];
    ...
    corrections = vecnorm(rv_orb(1:3,:) - rv_orb_required_initial(1:3,:)) > formation.tracking_error;
    ...
    %% Control
    if sum(corrections(2,:)) == 0
        maneuvers_out(:,formation.N_sats) = 0;
    else    
        for i = 1:formation.N_sats
            if corrections(2,i) == 1

                 [t_span, dV_sma] = calculate_sma_correction_dV(rv_ECI(i*6-5:i*6,1), rv_ECI_required(:,i), consts);

                 maneuvers_new = [corrections(1,i); 0; t_span; dV_sma; zeros(6,1)];
        ...
        %% Correction cycle
        while sum(maneuvers(2,:) == 3) < sum(corrections(2,:))
            ...
            if t_maneuvering ~= T_local
                ...
                t = [t; t_vec_local(2:end)];
                rv_ECI_local = rv_ECI_local';
                rv_ECI = [rv_ECI, rv_ECI_local(:,2:end)];
                T_local = t(end);
            ...
            switch maneuvers(2,1)
                ...
                maneuvers(7:9,1) = dV1_Vaddi;
                maneuvers(10:12,1) = dV2_Vaddi;
...
%% Calculating spent fuel 
    if mode ~= 3
        ...
        maneuvering_sats(:,1) = maneuvers_out(:,1)-1;
        sats_dV = maneuvers_out(:,2:end);
        sats_dV = sum(sats_dV,2);
        maneuvers_out = zeros(formation.N_active_sats,1);
        for i = 1:length(maneuvering_sats)
            maneuvers_out2(maneuvering_sats(i),1) = sats_dV(i);
        end
        
        formation.fuel_level = write_off_fuel(formation.fuel_level, maneuvering_sats, sats_dV, spacecraft, consts);
    else
        if isempty(maneuvers)
            maneuvers_out2 = 0;
        else
            maneuvers_out2 = sum([vecnorm(maneuvers(4:6,:)), vecnorm(maneuvers(7:9,:)), vecnorm(maneuvers(10:12,:))]);
        end
    end
...
%% Visualization
    ...
    legend('\deltar_x', '\deltar_y', '\deltar_z');
