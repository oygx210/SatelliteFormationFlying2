function [t_vec, rv_ECI, maneuvers, formation_fuel_level] = continuous_control_rev(rv_ECI, consts, spacecraft, formation, mode)
    % mode = 11 - continuous control for post-correction after impulsive
    % maneuvers (no matter if the tracking error threshold is reached the satellites are contolled)
    
    % mode = 12 - continuous control relative orbit for maintenance
    
    % mode = 21 - same as 11 + visualization on
    % mode = 22 - same as 12 + visualization on
...
%   disp('    Continuous control');
    ...
    for i = 1:formation.N_sats
        rv_orb(:,i) = ECI2orb(rv_ECI(1:6), rv_ECI(i*6-5:i*6), consts);
    end

    if mode == 11 || mode == 21
        maneuvering_satellites = ones(formation.N_sats,1);
    else
        maneuvering_satellites = vecnorm(rv_orb(1:3,:) - rv_orb_predicted(1:3,:)) > formation.tracking_error;
    end
